...
Use the redacted debug mode for support. It prints step names, HTTP status, and cloud/region only — never secrets, including Old/New credentials.
...
Do **not** run this script under `Set-PSDebug -Trace` or PowerShell's common `-Debug` for support. Tracing is forced off at startup (`Set-PSDebug -Off`) so credentials stay out of logs. `Set-PSDebug -Trace` can leak `client_secret`, tokens, `Authorization` headers, and oauth request bodies.
