        ...
        if ($statusCode -in @(301, 302, 303, 307, 308)) {
            ...
            try {
                if ($strippedHeaders.Count -gt 0) {
                    $null = Invoke-WebRequest @WebRequestParams -Uri $redirectUri.AbsoluteUri -UseBasicParsing -Method 'GET' -Headers $strippedHeaders -OutFile $Outfile
                }
                else {
                    $null = Invoke-WebRequest @WebRequestParams -Uri $redirectUri.AbsoluteUri -UseBasicParsing -Method 'GET' -OutFile $Outfile
                }
            }
            catch {
