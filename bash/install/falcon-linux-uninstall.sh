...
EOF
    ...
    ssm_curl_rc=$?
    falcon_debug "aws_ssm_parameter step=response curl_exit=${ssm_curl_rc} param=${param_name} region=${aws_my_region:-unset}"
    handle_curl_error "$ssm_curl_rc"
...
get_falcon_credentials() {
    if [ -z "$FALCON_ACCESS_TOKEN" ]; then
        ...
        if [ -n "$FALCON_CLIENT_ID" ]; then
            falcon_debug "get_falcon_credentials source=env cloud=${FALCON_CLOUD:-unset}"
        elif [ -n "$aws_instance" ]; then
            falcon_debug "get_falcon_credentials source=ssm cloud=${FALCON_CLOUD:-unset}"
        else
            falcon_debug "get_falcon_credentials source=missing cloud=${FALCON_CLOUD:-unset}"
        fi
        cs_falcon_client_id=$(
            if [ -n "$FALCON_CLIENT_ID" ]; then
